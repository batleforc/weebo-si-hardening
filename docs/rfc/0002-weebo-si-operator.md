...
`resource` is the Kubernetes kind of the *subject* the decision was made about, and it is the
chassis that supplies it: `Subject::resource()`, carried into `FeatureOutcome` by `admit` and
read from there by both metrics. It is not a value an adapter chooses — see the *Changelog* for
